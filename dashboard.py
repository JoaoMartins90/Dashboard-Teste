...
fig_mapa_vendas = px.scatter_geo(receita_estado,
                                  lat = 'lat',
                                  lon = 'lon',
                                  scope = 'south america',
                                  size = 'Preço',
...
fig_vendas_mensal.update_layout(yaxis_title = 'Quantidades de vendas')
...
fig_vendas_estados = px.bar(vendas_estados.head(),
                             ...
                             text_auto = True,
                             title = 'Top 5 estados')
fig_vendas_estados.update_layout(yaxis_title = 'Quantidades de vendas')
...
with aba3:
    qt_vendedores = st.slider('Quantidade de Vendedores',2,10,5)
    #qt_vendedores = st.number_input('Quantidade de vendedores', 2, 10, 5)
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'))
        fig_receita_vendedores = px.bar(vendedores[['sum']].sort_values('sum', ascending = False).head(qt_vendedores),
                                        x = 'sum',
                                        y = vendedores[['sum']].sort_values('sum', ascending = False).head(qt_vendedores).index,
                                        text_auto = True,
                                        title = f'Top {qt_vendedores} vendedores (receita)')
        st.plotly_chart(fig_receita_vendedores, use_container_width = True)

    with coluna2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]))
        fig_vendas_vendedores = px.bar(vendedores[['count']].sort_values('count', ascending = False).head(qt_vendedores),
                                        x = 'count',
                                        y = vendedores[['count']].sort_values('count', ascending = False).head(qt_vendedores).index,
                                        text_auto = True,
                                        title = f'Top {qt_vendedores} vendedores (quantidade de vendas)')
